...
public class POSController : Controller
{
    ...
    public JsonResult CancelReservation([FromBody]ReservationCreateModel request)
    {
        ...
        catch (Exception e)
        {
            response.Success = false;
            response.status = -1;
            response.Message = "OcurriÛ un error en la consulta: " + e.Message;
    ...
    public JsonResult UpdateCustomerName([FromBody] OrderInfoModel model)
    {
        ...
        try
        {
            ...
            if (status == 1)
            {
                response.Message = "No se encontrÛ la orden con el ID proporcionado.";
            }
            else
            {
                response.Message = "Se actualizÛ el nombre del cliente exitosamente.";
            }
        ...
        {
            response.Success = false;
            response.Message = "OcurriÛ un error al actualizar el nombre del cliente: " + ex.Message;
            response.status = -1;
    ...
    public JsonResult SubmitConduceOrders([FromBody] SubmitConduceOrdersRequest request, [FromBody] int stationId)
    {
        ...
        if (request.Orders.Count == 0)
        {
            response.status = 1;
            response.Success = false;
            response.Message = "No se encontraron Ûrdenes para conduce";
        ...
        catch (Exception ex)
        {
            response.Success = false;
            response.status = -1;
            response.Message = "OcurriÛ un error al realizar la operaciÛn: " + ex.Message;
    ...
    public JsonResult Kiosk(int stationId, int orderId = 0)
    {
        ...
        try
        {
            ...
            if (station == null)
            {
                throw new Exception("EstaciÛn no existe");
            }
