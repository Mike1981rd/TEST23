...
namespace AuroraPOS.Security
{
    public class Encriptacion
    {
        ...
        private static byte[] SALT_BYTES = new byte[] {
         162,
         27,
         98,
         1,
        ...
        private static Algorithm _algorithm = Algorithm.None;
        ...
        public static bool DecryptString()
        {
            ...
            _content = UTF8Encoding.UTF8.GetString(clearText);
        ...
        public static bool EncryptFile(string Filename, string Target)
        {
            ...
            byte[] encodedBytes = UTF8Encoding.UTF8.GetBytes(encodedString);
        ...
        /// <summary>
        /// Crea una clave hash del texto suministrado por parámetros
        /// </summary>
        /// <param name="Source"></param>
        /// <returns></returns>
        /// <remarks>
        /// Útil para no tener que almacenar contraseñas de usuarios en la base de datos del programa
        /// la contraseña suministrada por los usuarios podrá pasar por este mismo proceso y compararse las claves hash para determinar
        /// si la contraseña es válida
        /// </remarks>
        public static string CreateHash(string Source)
        {

            EncryptionAlgorithm = Algorithm.SHA512;
            Encoding = EncodingType.HEX;

            string s = null;

            if (GenerateHash(Source))
            {
                s = Content;
                Clear();
        ...
        /// <summary>
        /// Crea una clave hash MD5 Base64 del texto suministrado por parámetros
        /// </summary>
        /// <param name="Source"></param>
        /// <returns></returns>
        /// <remarks>
        /// </remarks>
        public static string CreateHashMD5(string Source)
        {

            EncryptionAlgorithm = Algorithm.MD5;
            Encoding = EncodingType.BASE_64;
        ...
        /// <summary>
        /// Cifra un texto dado mediante una clave privada
        /// </summary>
        /// <param name="TextoPlano">Texto claro</param>
        /// <param name="Clave">Clave privada</param>
        /// <returns>Texto cifrado</returns>
        /// <remarks>
        /// Se debe usar cuando se necesita cifrar alguna información que más tarde debe obtenerse nuevamente en texto claro
        /// </remarks>
        public static string Encrypt(string TextoPlano, string Clave)
        {

            EncryptionAlgorithm = Algorithm.Rijndael;
            Encoding = EncodingType.BASE_64;
            Key = Clave;

            string s = null;

            if (EncryptString(TextoPlano))
            {
                s = Content;
                Clear();
        ...
        /// <summary>
        /// Descifra un texto dada la clave privada
        /// </summary>
        /// <param name="TextoCifrado">Texto cifrado</param>
        /// <param name="Clave">Clave privada</param>
        /// <returns>Texto descifrado</returns>
        /// <remarks>
        /// Se debe usar para descifrar una información anteriormente cifrada con la clave privada suministrada
        /// </remarks>
        ...
        /// <summary>
        /// Encripta una clave privada usando funciones del API de Windows
        /// </summary>
        ...
        public static string SavePrivateKey(string PrivateKey)
        {
            ...
            DATA_BLOB bbPlainText = default(DATA_BLOB);
            DATA_BLOB bbCipherText = default(DATA_BLOB);
            DATA_BLOB bbEntropy = default(DATA_BLOB);
            CRYPTPROTECT_PROMPTSTRUCT pmt = new CRYPTPROTECT_PROMPTSTRUCT();

            //Initialize the pmt structure
            pmt.cbSize = Marshal.SizeOf(pmt);
            pmt.hwndApp = IntPtr.Zero;
            pmt.szPrompt = String.Empty;

            //Convert the plaintext into a byte array, and copy to global memory
            bytPlainText = uEncode.GetBytes(PrivateKey);
            bbPlainText.pbData = Marshal.AllocHGlobal(bytPlainText.Length);

            if (bbPlainText.pbData.ToInt32() == 0)
            {
                throw new ApplicationException("Global alloc failed. Fallo al proteger la clave privada.");
            ...
            blnSuccess = CryptProtectData(ref bbPlainText, "", ref bbEntropy, IntPtr.Zero, ref pmt, 1, ref bbCipherText);

            if (blnSuccess == false)
            {
                throw new ApplicationException("CryptProtect failed. Fallo al proteger la clave privada.");

            }

            //The result is stored in a block of memory. Convert this block to a string
            bytCipherText = new byte[bbCipherText.cbData + 1];
            Marshal.Copy(bbCipherText.pbData, bytCipherText, 0, bbCipherText.cbData);
            Marshal.FreeHGlobal(bbPlainText.pbData);

            //Devolver el string de la clave privada protegida
            return System.Convert.ToBase64String(bytCipherText);

        }

        /// <summary>
        /// Desencripta una clave privada usando funciones del API de Windows
        /// </summary>
        /// <param name="CipherPrivateKey"></param>
        /// <returns></returns>
        /// <remarks></remarks>
        public static string LoadPrivateKey(string CipherPrivateKey)
        {
            ...
            bool blnSuccess = false;
            DATA_BLOB bbPlainText = default(DATA_BLOB);
            DATA_BLOB bbCipher = default(DATA_BLOB);
            DATA_BLOB bbEntropy = default(DATA_BLOB);
            CRYPTPROTECT_PROMPTSTRUCT pmt = new CRYPTPROTECT_PROMPTSTRUCT();
            ...
            //Copy the cipher key into a byte array and store it in global memory
            bytCipherText = System.Convert.FromBase64String(CipherPrivateKey);
            bbCipher.pbData = Marshal.AllocHGlobal(bytCipherText.Length);

            if (bbCipher.pbData.ToInt32() == 0)
            {
                throw new ApplicationException("Global alloc failed. Fallo al recuperar la clave privada.");
            ...
            blnSuccess = CryptUnprotectData(ref bbCipher, string.Empty, ref bbEntropy, IntPtr.Zero, ref pmt, 1, ref bbPlainText);

            if (blnSuccess == false)
            {
                throw new ApplicationException("CryptUnprotect failed. Fallo al recuperar la clave privada.");

            }

            //The result is stored in a block of memory. Convert to a string and return to the user
            Marshal.FreeHGlobal(bbCipher.pbData);

            byte[] plainText = new byte[bbPlainText.cbData];
        ...
        {
            return _Encrypt(UTF8Encoding.UTF8.GetBytes(Content));
        }
        ...
        private static byte[] _Decrypt(byte[] Content)
        {
            ...
            string encText = UTF8Encoding.UTF8.GetString(Content);
        ...
        {
            return _Decrypt(UTF8Encoding.UTF8.GetBytes(Content));
        }
        ...
        private static byte[] ComputeHash(HashAlgorithm Provider, string plainText)
        {
            ...
            byte[] hash = Provider.ComputeHash(UTF8Encoding.UTF8.GetBytes(plainText));
            Provider.Clear();
            return hash;
        }
        ...
        private static byte[] SymmetricEncrypt(SymmetricAlgorithm Provider, byte[] plainText, string key, int keySize)
        {
            ...
            byte[] keyStream = DerivePassword(key, Convert.ToInt32(keySize / 8));
        ...
        private static byte[] RSAEncrypt(byte[] plainText)
        {
            ...
            int blockCount = Convert.ToInt32(Math.Floor(Convert.ToDecimal(plainText.Length / RSA_BLOCKSIZE)));
            ...
            for (int blockIndex = 0; blockIndex <= blockCount - 1; blockIndex++)
            {
                ...
                System.Array.Resize(ref result, originalResultLength + encryptedBlock.Length);
        ...
        private static byte[] RSADecrypt(string encText)
        {
            ...
            int maxBytes = Convert.ToInt32(encText.Length / 2);
            ...
            int blockCount = Convert.ToInt32(maxBytes / RSA_DECRYPTBLOCKSIZE);

            //Initialize the result buffer
            byte[] result = new byte[encText.Length];
        ...
        {
            int numBytes = Convert.ToInt32(Hex.Length / 2);
